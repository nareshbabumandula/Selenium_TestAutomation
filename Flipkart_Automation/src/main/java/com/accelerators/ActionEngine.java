 ...
 */
	...
	public boolean type(By locator, String testdata, String locatorName)
		...
		try {
			...
			if (flag) {
				...
				String asText[] = errMessage.split("}");
				failureReport("Type Data in "+locatorName,"Data typing action is not performed on \""+locatorName+"\" with data \""+testdata+"\"" +  " - since " +asText[0]);
			}
		}
		return flag;
	}
	...
	public Boolean switchToNewWindow()
		...
		try {
			...
			if (flag) {
				SuccessReport("SelectWindow","Window is Navigated with title:" + driver.getTitle());				
			} else {
	...
	public String getTitle() throws Throwable {
		...
		{
			failureReport("Title ","Title of the page is empty");
		}
		return text;
	}
	...
	public  boolean waitForFrameToLoadAndSwitchToIt(By by, String LocatorName) throws Throwable{
		...
		cmdStartTime=System.currentTimeMillis();
		try {
			//wait(ExpectedConditions.frameToBeAvailableAndSwitchToIt((by));
...
//				SuccessReport("Wait for element Present",locatorName +" Element is present");
	...
	public void sort(By locator, String locatorname)
	{
	...
	public boolean isclickable(By locator, String locatorName)
			throws Throwable {
		cmdStartTime=System.currentTimeMillis();
		boolean flag = false;
		try {
			String source = driver.findElement(locator).getAttribute("src");
			if (source!=null) {
				flag = true;
			}
		} catch (Exception e) {
			...
			if (flag) {
				SuccessReport("Click on "+locatorName, locatorName + " is clickable");
				//return true;
			} else if(!flag) {
				failureReport("Click on "+locatorName, locatorName + " is not clickable");
			}
